...
describe('adaptFormDataToPayload', () => {
  ...
  it('should return null for empty documentType', () => {
    const formData: any = {
      ruralProperties: { mailingAddress: {} },
      landHoldersInformation: {},
      registrarDetails: {},
    ...
    const payload = adaptFormDataToPayload(formData)
    expect(payload.documents[0].documentType).toBe(null)
  })

  it('should return null for empty string attributes', () => {
    const formData: any = {
      ruralProperties: { mailingAddress: {} },
      landHoldersInformation: {},
      registrarDetails: {},
    ...
    const payload = adaptFormDataToPayload(formData)
    const attributes = payload.documents[0].attributes
    expect(attributes.find((a) => a.name === 'holdingType').value).toBe(null)
    expect(attributes.find((a) => a.name === 'deed').value).toBe('valid')
    expect(attributes.find((a) => a.name === 'book').value).toBe(null)
  })
})
